...
%\end{remark*}
\begin{ex} Обратное, вообще говоря, неверно. Действительно, 
  % TODO example
\end{ex}

\begin{theorem}
  Если последовательность $\{ \xi_n \}$ монотонно возрастает (убывает) и $\xi_n \toP \xi$, то
	\[
		\xi_n \toPN \xi.
	\]
\end{theorem}
...
\begin{theorem}[закон больших чисел Чебышёва]
	Пусть $ \xi_1, \xi_2, \ldots $ --- независимые случайные величины и существует
	такая константа $ c > 0 $, что все $\mathsf D \xi_i \leqslant c$,
	$i=\overline{1,2,\ldots}$. Тогда при любом $ \varepsilon > 0 $  
	\[
