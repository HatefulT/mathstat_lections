...
\begin{proof}[Доказательство теоремы \ref{the:1}]
  Рассмотрим центрированную и нормированную выборку
	...
	Компоненты вектора $ \bar X_n' $ независимы и распределены по стандартному
	нормальному закону. Значит,  
	\begin{gather*}
		\bar X' = \frac{1}{n} \sum\limits_{k=1}^n X_k' = \frac{1}{n} \sum \frac{(X_k
		- a)}{\sigma} = \frac{\bar X - a}{\sigma}, \\
  p_{\bar X'} (\bar x) = \frac{1}{(\sqrt{2\pi})^n} \exp\left(-\frac{1}{2} \bar X^T \bar
	X\right).
\end{gather*}
...
\[
	\frac{S^2(n-1)}{\sigma^2} = \sum_{k=1}^n Y^2_k \sim \chi^2(n-1).
\]
